...
//	error_reporting(0);
  ...
  if (true) {
    ...
    // if (isset($_REQUEST["user_id"]))
      // $where .= " AND aktywny = 1";
    $query = "SELECT `id`, `par_id`, `nazwa`, `opis`, `aktywny`, `nested_folders` as folders, `nested_files` as files FROM kart_pr_projekty".$where." ORDER BY id ASC;";
    // echo $query;
...
array_push($times,microtime(true));	
    ...
    $date = new DateTime();
    echo 'var o_serv_epoch = '.$date->getTimestamp().'000;';
    echo 'var o_projekty = '.json_encode($projekty).';';
    echo 'var o_zadania = '.json_encode($zadania).';';
