...
////http://luban.danse.us/jazzclub/javascripts/jquery/jsTree/reference/_examples/5_others.html
	...
	if (isset($_REQUEST["usun"]) || isset($_REQUEST["dodaj"]) || isset($_REQUEST["dodaj_mpk"]) || isset($_REQUEST["label"]) || isset($_REQUEST["rename"]) || isset($_REQUEST["active"]) || isset($_REQUEST["template"])){
		...
		if (isset($_REQUEST["active"])){
			$id = $_REQUEST["active"];
			$query = "UPDATE `kart_pr_projekty` SET `aktywny` = not aktywny WHERE `id`=$id;";
			if ($mysqli->query($query))
...
//	echo $query;
	...
	$query = "SELECT * FROM `kart_pr_projekty` WHERE deleted = 0 and par_id is not null ORDER BY lvl, nazwa;";
/*
	"SELECT * 
...
/*
	...
	for (var si in pnu)
		for (var pi in pnu[si])
			for (var ei in pnu[si][pi].etap) {
				var e = pnu[si][pi].etap[ei];
				if (e && pi == 5) {
					for (var zi in e) {
						var si2 = si;
						if (si2 == "Z")
							si2 = "";
						// console.log(pi+si+"/"+ei+"/"+zi);
						// console.log("UPDATE `kart_pr_zadania` SET `termin`="+e[zi].termin+" WHERE typ = 'PNU' AND zlecenie like '"+pi+si2+"/"+ei+"/"+zi+"' AND termin IS NULL;");
						temp_text += "UPDATE `kart_pr_zadania` SET `termin`="+e[zi].termin+" WHERE typ = 'PNU' AND zlecenie like '"+pi+si2+"/"+ei+"/"+zi+"' AND termin IS NULL; \n";
					}
				}
			}
